#ifndef STATIC_REDUNDANCY_CONTROLLER_H
#define STATIC_REDUNDANCY_CONTROLLER_H
#include "../fec.h"
...
// 1 packet == 1 symbol
#define MAX_SLOTS (MAX_SENDING_WINDOW_SIZE + 5) // TODO: ???

#define GRANULARITY 1000000 // TODO: ???

#define NUMBER_OF_SOURCE_SYMBOLS_TO_PROTECT_IN_FB_FEC 1 // TODO: ???

#define COMPUTE_AD_MD_FAST_VERSION true // TODO: ???

typedef uint64_t buffer_elem_t;

typedef enum static_packet_type {
    new_rlnc_packet,
    fec_packet,
    fb_fec_packet,
    nothing,
} static_packet_type_t;
...
#define AD_MD_NONE UINT64_MAX // TODO: ???
...
typedef struct {
    ...
    protoop_arg_t addons_states[10]; // FIXME: free

} static_redundancy_controller_t;
...
static __attribute__((always_inline)) buffer_t* create_buffer_old(picoquic_cnx_t* cnx, int max_size) {
    if (max_size > MAX_SLOTS || !max_size)
        return NULL;
...
static __attribute__((always_inline)) bool buffer_next(picoquic_cnx_t* cnx, buffer_rbt_t* buffer, buffer_elem_t slot,
                                                       buffer_elem_t* next) {
    rbt_key key = 0;
...
static __attribute__((always_inline)) bool first_md_id_covered_by(picoquic_cnx_t* cnx,
...
static __attribute__((always_inline)) buffer_elem_t
add_elem_to_buffer_with_val(picoquic_cnx_t* cnx, buffer_rbt_t* buffer, buffer_elem_t slot, rbt_val val) {
    buffer_elem_t ret = 0;
    if (rbt_size(cnx, &buffer->collection) == buffer->max_size) {
        rbt_delete_and_get_min(cnx, &buffer->collection, (rbt_key*)&ret, NULL);
    }
...
// returns the removed element
static __attribute__((always_inline)) uint64_t add_elem_to_buffer(picoquic_cnx_t* cnx, buffer_rbt_t* buffer,
                                                                  buffer_elem_t slot) {
...
static __attribute__((always_inline)) bool remove_elem_from_buffer(picoquic_cnx_t* cnx, buffer_rbt_t* buffer,
                                                                   buffer_elem_t slot) {
    if (rbt_is_empty(cnx, &buffer->collection))
        return false;
...
static __attribute__((always_inline)) bool dequeue_elem_from_buffer(picoquic_cnx_t* cnx, buffer_rbt_t* buffer,
                                                                    buffer_elem_t* elem) {
    if (rbt_is_empty(cnx, &buffer->collection) == 0)
        return false;
...
static __attribute__((always_inline)) bool is_elem_in_buffer(picoquic_cnx_t* cnx, buffer_rbt_t* buffer,
                                                             buffer_elem_t slot) {
    return rbt_contains(cnx, &buffer->collection, (rbt_key)slot);
}

static __attribute__((always_inline)) bool get_val_from_buffer(picoquic_cnx_t* cnx, buffer_rbt_t* buffer,
                                                               buffer_elem_t slot, rbt_val* val) {
    return (bool)rbt_get(cnx, &buffer->collection, (rbt_key)slot, val);
}

static __attribute__((always_inline)) bool is_buffer_empty_old(buffer_t* buffer) { return buffer->current_size == 0; }
...
static __attribute__((always_inline)) int64_t window_size(fec_window_t* window) { return window->end - window->start; }
...
static __attribute__((always_inline)) bool remove_symbol_from_window(fec_window_t* window,
                                                                     window_source_symbol_id_t id) {
    if (is_window_empty(window) || window->start != id)
        return false;
    window->start++;
    return true;
}

static __attribute__((always_inline)) buffer_elem_t buffer_get_first(picoquic_cnx_t* cnx, buffer_rbt_t* buffer) {
    if (!is_buffer_empty(cnx, buffer)) {
        return (buffer_elem_t)rbt_min_key(cnx, &buffer->collection);
    }
    return 0;
}
...
static __attribute__((always_inline)) void add_elem_to_history(slots_history_t* history, int64_t slot,
                                                               window_packet_metadata_t md) {
...
// returns true if elem has been removed from history and false otherwise
...
static __attribute__((always_inline)) int get_window_sent_at_slot(picoquic_cnx_t* cnx,
                                                                  static_redundancy_controller_t* controller,
                                                                  slots_history_t* history, int64_t slot,
                                                                  fec_window_t* w) {
    if (!is_slot_in_history(history, slot))
        return -1;
    window_packet_metadata_t md = history->sent_windows[((uint64_t)slot) % ((uint64_t)history->max_size)].md;
    if (is_elem_in_buffer(cnx, controller->fec_slots, slot) || is_elem_in_buffer(cnx, controller->fb_fec_slots, slot)) {
...
static __attribute__((always_inline)) int history_get_sent_slot_metadata(slots_history_t* history, int64_t slot,
    ...
    if (history->sent_windows[idx].slot != slot)
        return -1;
    *md = history->sent_windows[idx].md;
    return 0;
}

static __attribute__((always_inline)) static_redundancy_controller_t*
create_static_redundancy_controller(picoquic_cnx_t* cnx, uint32_t m) {
    static_redundancy_controller_t* controller = my_malloc(cnx, sizeof(static_redundancy_controller_t));
    if (!controller)
        return NULL;
    my_memset(controller, 0, sizeof(static_redundancy_controller_t));
    controller->m = m;
...
destroy_static_redundancy_controller(picoquic_cnx_t* cnx, static_redundancy_controller_t* controller) {
    // FIXME: free other entities???
    destroy_buffer(cnx, controller->acked_slots);
    destroy_buffer(cnx, controller->nacked_slots);
    my_free(cnx, controller);
}
...
static __attribute__((always_inline)) bool EW(picoquic_cnx_t* cnx, picoquic_path_t* path,
                                              static_redundancy_controller_t* controller, uint64_t granularity,
                                              fec_window_t* current_window, uint64_t current_time) {
    ...
    *malloced_window = *current_window;
    ...
    int retval = run_noparam(cnx, "static_ew", 5, args, NULL);
    my_free(cnx, malloced_window);
    return retval;
}

static __attribute__((always_inline)) int64_t threshold(picoquic_cnx_t* cnx, picoquic_path_t* path,
                                                        static_redundancy_controller_t* controller,
                                                        uint64_t granularity, uint64_t current_time) {
    ...
    return run_noparam(cnx, "static_threshold", 4, args, NULL);
}

static __attribute__((always_inline)) bool below_threshold(picoquic_cnx_t* cnx, picoquic_path_t* path,
                                                           static_redundancy_controller_t* controller,
                                                           uint64_t granularity, uint64_t current_time) {
...
static __attribute__((always_inline)) bool is_fec_or_fb_fec(picoquic_cnx_t* cnx,
                                                            static_redundancy_controller_t* controller, uint64_t slot) {
    return is_elem_in_buffer(cnx, controller->fec_slots, slot) ||
           is_elem_in_buffer(cnx, controller->fb_fec_slots, slot);
}

// returns true if slot not in controller->acked_slots and not in controller->nacked_slots
static __attribute__((always_inline)) bool
is_slot_in_flight(picoquic_cnx_t* cnx, static_redundancy_controller_t* controller, uint64_t slot) {
    return !(is_elem_in_buffer(cnx, controller->acked_slots, slot) ||
             is_elem_in_buffer(cnx, controller->nacked_slots, slot));
}

static __attribute__((always_inline)) bool
is_in_flight_and_fec(picoquic_cnx_t* cnx, static_redundancy_controller_t* controller, uint64_t slot) {

    return is_fec_or_fb_fec(cnx, controller, slot) && is_slot_in_flight(cnx, controller, slot);
}

// whether given slot "is_in_flight_and_fec" and protect current_window (at least one element)
static __attribute__((always_inline)) bool is_ad(picoquic_cnx_t* cnx, static_redundancy_controller_t* controller,
...
// removes [from; to_included] source symbol ids from controller->md_ids rbt buffer
static __attribute__((always_inline)) void prune_md_buffer(picoquic_cnx_t* cnx,
                                                           static_redundancy_controller_t* controller,
                                                           window_source_symbol_id_t from,
                                                           window_source_symbol_id_t to_included) {
...
// remove all slots from controller->ad_slots whose window doesn't intersect with current_window
static __attribute__((always_inline)) void
prune_ad_buffer(picoquic_cnx_t* cnx, static_redundancy_controller_t* controller, fec_window_t* current_window) {
    fec_window_t sent_window;
...
// computes Added Degrees of freedom == size of controller->ad_slots
static __attribute__((always_inline)) uint32_t
compute_ad(picoquic_cnx_t* cnx, static_redundancy_controller_t* controller, fec_window_t* current_window) {
    int err = 0;
    ...
    for (uint64_t slot = first_slot + 1; slot <= controller->last_sent_slot; slot++) {
        ...
        if (!err && is_fec_or_fb_fec(cnx, controller, slot) && is_slot_in_flight(cnx, controller, slot) &&
    ...
    PROTOOP_PRINTF(cnx, "AD = %u, CACHED AD = %u, AD SIZE = %u\n", ad, controller->cached_ad,
                   buffer_get_size(cnx, controller->ad_slots));

    return ad;
}

// returns md+1 (and why we need this?)
static __attribute__((always_inline)) uint32_t inc(uint32_t md) { return md + 1; }

// whether source symbol with given id is nacked (whose slot is in controller->nacked_slots->collection)
static __attribute__((always_inline)) bool is_md(picoquic_cnx_t* cnx, static_redundancy_controller_t* controller,
                                                 window_source_symbol_id_t id) {
    rbt_val val;
...
// computes Missing Degrees of freedom == size of controller->md_ids
static __attribute__((always_inline)) uint32_t
compute_md(picoquic_cnx_t* cnx, static_redundancy_controller_t* controller, fec_window_t* current_window) {
    if (window_size(current_window) == 0) {
        return 0;
    }
    if (COMPUTE_AD_MD_FAST_VERSION) {
    ...
    for (window_source_symbol_id_t id = current_window->start; id < current_window->end; id++) {
        ...
        //        bool is_contained = is_elem_in_buffer(cnx, controller->nacked_slots, slot);
        uint64_t is_contained = rbt_contains(cnx, &controller->nacked_slots->collection, slot) &&
                                !rbt_contains(cnx, &controller->acked_slots->collection, slot);
        if (is_contained) {
...
static __attribute__((always_inline)) bool
check_protect_condition(picoquic_cnx_t* cnx, picoquic_path_t* path, static_redundancy_controller_t* controller,
                        uint64_t granularity, window_source_symbol_id_t* first_id_to_protect,
                        uint16_t* n_symbols_to_protect, uint16_t* n_repair_symbols, fec_window_t* current_window,
    ...
    *malloced_window = *current_window;
    ...
    bool retval = run_noparam(cnx, "fec_protect_condition", 5, args, out) ? true : false;
    *first_id_to_protect = out[0];
    *n_symbols_to_protect = out[1];
    *n_repair_symbols = out[2];
    ...
    if (state->n_reserved_id_or_repair_frames > 0) {
        ...
        *n_repair_symbols = 1;
        ...
        if (!controller->flush_dof_mode && is_buffer_empty_old(controller->what_to_send)) {
            ...
            controller->flush_dof_mode = window_size(window) + to_add > MAX_SLOTS - 1;
        }
...
// changes state due to sending a packet
static __attribute__((always_inline)) void sent_packet(picoquic_cnx_t* cnx, uint64_t current_time,
                                                       picoquic_path_t* path, window_redundancy_controller_t c,
                                                       static_packet_type_t type, uint64_t slot,
                                                       window_packet_metadata_t md, fec_window_t* current_window) {
    static_redundancy_controller_t* controller = (static_redundancy_controller_t*)c;
    ...
    if (current_time - controller->last_bandwidth_check_timestamp_microsec > get_path(path, AK_PATH_SMOOTHED_RTT, 0)) {
        controller->last_bytes_sent_sampling_period_microsec =
            current_time - controller->last_bandwidth_check_timestamp_microsec;
...
static __attribute__((always_inline)) void slot_acked(picoquic_cnx_t* cnx, window_redundancy_controller_t c,
    ...
    //    buffer_get_first(cnx, controller->md_ids));
    window_packet_metadata_t md;
...
static __attribute__((always_inline)) void free_slot_without_feedback(picoquic_cnx_t* cnx,
                                                                      window_redundancy_controller_t c) {
    static_redundancy_controller_t* controller = (static_redundancy_controller_t*)c;
    controller->last_feedback = available_slot_reason_none;
}

static __attribute__((always_inline)) void slot_nacked(picoquic_cnx_t* cnx, window_redundancy_controller_t c,
                                                       fec_window_t* current_window, uint64_t slot) {
    static_redundancy_controller_t* controller = (static_redundancy_controller_t*)c;
    controller->n_received_feedbacks++;
    ...
    if (md.source_metadata.number_of_symbols > 0 || (md.repair_metadata.is_fb_fec)) {
        ...
        if (md.source_metadata.number_of_symbols > 0 && is_slot_in_flight(cnx, controller, slot) &&
            window_contains(current_window, md.source_metadata.first_symbol_id)) {
            controller->cached_md++;
...
// whether controller->lost_and_non_fec_retransmitted_slots is empty
static __attribute__((always_inline)) bool retransmission_needed(picoquic_cnx_t* cnx,
                                                                 window_redundancy_controller_t c) {
    static_redundancy_controller_t* controller = (static_redundancy_controller_t*)c;
